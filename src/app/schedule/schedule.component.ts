...
export class ScheduleComponent implements OnInit {
  ...
  private baseUrl = 'https://spring-timesheet.azurewebsites.net'; // Update with your actual backend URL
  ...
  ngOnInit() {
    ...
    console.log('User email from cookie:', this.userEmail); // Debugging statement
    this.setMinMaxDates();
  ...
  onClientFocus() {
    this.filteredClients = this.allClients.map(client => `${client.clientId}: ${client.clientName}`);
  ...
  onProjectFocus() {
    if (this.checkWorkRequiresProject()) {
      this.filteredProjects = this.allProjects.map(project => `${project.projectId}: ${project.projectName}`);
